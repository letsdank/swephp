...
class SwephLib extends SweModule
{
    ...
    function swi_coortrf(array $xpo, array &$xpn, float $eps): void
    {
        $sineps = sin($eps);
        $coseps = cos($eps);
        $this->swi_coortrf2($xpo, $xpn, $sineps, $coseps);
    }
    ...
    function swi_cartpol_sp(array $x, array &$l): void
    {
        ...
        if ($x[0] == 0 && $x[1] == 0 && $x[2] == 0) {
            ...
            $this->swi_cartpol([$x[3], $x[4], $x[5]], $ll);
            $ll[2] = 0;
            for ($i = 0; $i <= 5; $i++)
                $l[$i] = $ll[$i];
            return;
    ...
    private function calc_deltat(float $tjd, int $iflag, float &$deltat, ?string &$serr): int
    {
        $tid_acc = 0.0;
        $denumret = 0;
        $ans = 0;
        // TODO: Remove ?? 0
        $deltat_model = $this->swePhp->sweph->swed->astro_models[SweModel::MODEL_DELTAT->value] ?? 0;
        if ($deltat_model == 0) $deltat_model = SweModelDeltaT::default();
        $epheflag = $iflag & self::SEFLG_EPHMASK;
        $otherflag = $iflag & ~self::SEFLG_EPHMASK;
        ...
        if ($iflag == -1) {
            ...
            $denum = $this->swePhp->sweph->swed->jpldenum;
            if ($epheflag & SweConst::SEFLG_SWIEPH)
                $denum = $this->swePhp->sweph->swed->fidat[Sweph::SEI_FILE_MOON]->sweph_denum;
            if ($this->swePhp->sweph->swi_init_swed_if_start() == 1 && !($epheflag & SweConst::SEFLG_MOSEPH)) {
                if ($serr)
            ...
            $tid_acc = $this->swePhp->sweph->swed->tid_acc;
        }
    ...
    public function swe_deltat_ex(float $tjd, int $iflag, ?string &$serr = null): float
    {
        ...
        if ($serr)
            $serr[0] = "\0";
        $this->calc_deltat($tjd, $iflag, $deltat, $serr);
        return $deltat;
    }

    public function swe_deltat(float $tjd): float
    {
        $iflag = $this->swi_guess_ephe_flag();
    ...
    {
        if (!$this->swePhp->sweph->swed->init_dt_done) {
            $this->swePhp->sweph->swed->init_dt_done = true;
            // no error message if file is missing
            if (($fp = $this->swePhp->sweph->swi_fopen(-1, "swe_deltat.txt", $this->swePhp->sweph->swed->ephepath)) == null &&
                ($fp = $this->swePhp->sweph->swi_fopen(-1, "sedeltat.txt", $this->swePhp->sweph->swed->ephepath)) == null)
                return self::TABSIZ;
    ...
    private function swi_guess_ephe_flag(): int
    {
        $iflag = SweConst::SEFLG_SWIEPH;
        // if jpl file is open, assume SEFLG_JPLEPH
        if ($this->swePhp->sweph->swed->jpl_file_is_open)
    ...
    {
        $iflag &= self::SEFLG_EPHMASK;
        if ($this->swePhp->sweph->swed->is_tid_acc_manual) {
            $tid_acc = $this->swePhp->sweph->swed->tid_acc;
            return $iflag;
        }
        if ($denum == 0) {
            ...
            if ($iflag & SweConst::SEFLG_SWIEPH) {
                if ($this->swePhp->sweph->swed->fidat[Sweph::SEI_FILE_MOON]->fptr != null)
                    $denum = $this->swePhp->sweph->swed->fidat[Sweph::SEI_FILE_MOON]->sweph_denum;
            }
        }
        switch ($denum) {
    ...
    private function swi_set_tid_acc(float $tjd_ut, int $iflag, int $denum, ?string &$serr = null): int
    {
        ...
        $retc = $this->swi_get_tid_acc($tjd_ut, $iflag, $denum, $denumret,
            $this->swePhp->sweph->swed->tid_acc, $serr);
